...
import { reconcileCurrentPlayers } from "../playerIdentity/service.js";
import { isYahooApiError, YahooErrorCode } from "../yahoo/errors.js";

export const playerIdentityRouter: Router = createRouter();
...
function sendError(res: Response, error: unknown): void {
  ...
  console.error("Player identity route failed.");
  res.status(502).json({ error: "identity_error", message: "Player identity reconciliation failed." });
}

playerIdentityRouter.get("/reconcile", async (_req: Request, res: Response) => {
  try {
    res.json(await reconcileCurrentPlayers());
  } catch (error) {
    sendError(res, error);
  }
});
