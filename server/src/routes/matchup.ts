...
import { getMatchupIntelligence } from "../matchup/service.js";

export const matchupRouter = Router();

matchupRouter.get("/", async (_req: Request, res: Response) => {
  try {
    res.json(await getMatchupIntelligence());
  } catch (error) {
    sendError(res, error);
  }
});
...
function sendError(res: Response, error: unknown): void {
  ...
  console.error("Matchup intelligence route failed.");
  res.status(502).json({ error: "matchup_error", message: "Matchup request failed." });
}
